...
import com.my13each.dooray.mcp.types.JoinChannelRequest
import com.my13each.dooray.mcp.types.JoinChannelResponseData
...
fun joinChannelTool(): Tool {
    return Tool(
        name = "dooray_messenger_join_channel",
        description = "두레이 메신저 채널에 멤버를 추가합니다.",
        inputSchema = Tool.Input(
            properties = buildJsonObject {
                putJsonObject("channel_id") {
                    put("type", "string")
                    put("description", "멤버를 추가할 채널의 ID")
                }
                putJsonObject("member_ids") {
                    put("type", "array")
                    put("description", "추가할 멤버 ID 목록")
                    putJsonObject("items") {
                        put("type", "string")
                    }
                }
...
fun joinChannelHandler(doorayClient: DoorayClient): suspend (CallToolRequest) -> CallToolResult {
    return { request ->
        try {
            ...
            when {
                ...
                memberIds == null || memberIds.isEmpty() -> {
                    ...
                    val joinChannelRequest = JoinChannelRequest(memberIds = memberIds)

                    val response = doorayClient.joinChannel(channelId, joinChannelRequest)

                    if (response.header.isSuccessful) {
                        val data = JoinChannelResponseData(
                            channelId = channelId,
                            memberIds = memberIds,
                        ...
                        val errorResponse = ToolException(
                            ...
                            message = "채널 가입 실패: ${response.header.resultMessage}",
            ...
            val errorResponse = ToolException(
                ...
                message = "채널 가입 중 오류가 발생했습니다: ${e.message}",
