...
- **🔗 ドライブ공유링크**: 공유링크生성、조회、수정、삭제 - 조직내외부사용자と安全にファイル共有
...
**ドライブに새しいフォルダを생성**します。親フォルダID、フォルダ名を指定してフォルダを생성할수있습니다。

#### 39. dooray_drive_copy_file

**ドライブファイルを別の場所にコピー**します。同じドライブ内또는다른ドライブへのコピーを지원합니다。

#### 40. dooray_drive_move_file

**ドライブファイルを別のフォルダに이동**します。ファイルの場所を변경する際に사용합니다。

### 🔗 ドライブ공유링크関連ツール（5개）

#### 41. dooray_drive_create_shared_link

**ファイルの공유링크를생성**합니다。공유범위（조직내/외부포함）と만료일을지정할수있습니다。

📌 **공유범위オプション:**
- `member`: 손님を제외한조직내사용자
- `memberAndGuest`: 조직내모든사용자（멤버+손님）
- `memberAndGuestAndExternal`: 내외부상관없이누구나

📌 **권한**: プロジェクト관리자와생성자만생성가능

#### 42. dooray_drive_get_shared_links

**ファイルに생성된모든공유링크를조회**합니다。관리자는모든링크를、일반사용자는자신이생성한링크만확인할수있습니다。유효한링크또는만료된링크를필터링하여조회가능합니다。

#### 43. dooray_drive_get_shared_link_detail

**특정공유링크の상세정보を조회**합니다。링크ID、생성일시、만료일시、생성자정보、실제공유링크URL、공유범위를확인할수있습니다。

#### 44. dooray_drive_update_shared_link

**특정공유링크を수정**합니다。만료날짜와공유범위를변경할수있습니다。

#### 45. dooray_drive_delete_shared_link

**특정공유링크를삭제**합니다。삭제된링크로는더이상파일에접근할수없으며、삭제작업은되돌릴수없습니다。

> 💡 **기술仕様**: Dooray Drive APIの307リダイレクト フローに対応。初期リクエストを `api.dooray.com` に送信し、307응답から `file-api.dooray.com` へのリダイレクトURLを取得して실제작업을실행합니다。
