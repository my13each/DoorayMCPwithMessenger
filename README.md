...
メッセンジャーチャンネルにメッセージを送信します。**メンション機能対応**: 特定ユーザーメンション `[@ユーザー名](dooray://組織ID/members/メンバーID "member")` または チャンネル全体メンション `[@Channel](dooray://組織ID/channels/チャンネルID "channel")` が使用可能。
